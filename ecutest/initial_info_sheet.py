...
D_ID = 'DID'
filename = 'DID_sheet'
filename_proposal = 'SID_22_proposal_sheet.xlsm'
...
def remove_characters_from_end(string, old_substring, new_substring):
...
def Initial_info(File_dir_proposal):
    ...
    for row in range(4,row_max):
        ...
        test_data = []
        for i in range(0,int(column_index_from_string('AN')-column_index_from_string('AA')),2):
            cur_col = i + start_col
    ...
    DID_file_name = remove_characters_from_end(File_dir_proposal,filename_proposal,new_file)
    print(DID_file_name)
    workbook.save(DID_file_name)

    # Specify the path and filename for the text file
    file_path = "ramcell.lab"
    ramcell_path = remove_characters_from_end(File_dir_proposal,filename_proposal,file_path)
    print(ramcell_path)

    # Open the file in write mode and save the string
    with open(ramcell_path, "w") as file:
